...
.get_basic <- function(data) {
  basic <- data %>%
    ...
    dplyr::summarise(
      ...
      mean_cdr3nt_length = mean(nchar(cdr3nt), na.rm = TRUE),
      convergence = mean(dplyr::n_distinct(cdr3nt), na.rm = TRUE),
      .groups = "drop"
    ) %>%
    dplyr::select(
      sample, cmv_status, reads_count, clonotype_count, mean_freq,
      geomean_freq, mean_cdr3nt_length, convergence
    )
  ...
  return(
    methods::new(
      ...
      mean_cdr3nt_length = dplyr::select(
        basic,
        sample,
        cmv_status,
        mean_cdr3nt_length
